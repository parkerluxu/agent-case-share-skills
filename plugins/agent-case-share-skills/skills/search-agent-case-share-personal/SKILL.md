...
- `get_my_case`: read a personal case by opaque `slug`, including its `attachments`, `reusableAssets`, models, integrations, prompts, and reproduction details.
...
4. Read selected results with `get_my_case` or `get_my_asset`. Use the case detail for its runtime/model configuration, integrations, prompts, and reproduction/verification information. Attachments are discovered through `get_my_case`, not personal asset search/list tools. Pass returned slugs and IDs unchanged.
