...
description: Search and read the current user's private Agent Case Share cases, case videos, case attachments, and reusable assets through the connected MCP server.
---
...
4. Read selected results with `get_my_case` or `get_my_asset`. Use the case detail for its videos, runtime/model configuration, integrations, prompts, and reproduction/verification information. Attachments are discovered through `get_my_case`, not personal asset search/list tools. Pass returned slugs, video IDs, asset IDs, and URLs unchanged.
